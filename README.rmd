...
```{r installlobbyr, eval = FALSE}
devtools::install_github("Lobbying-DisclosuRe/lobbyr")
```

+----------------------------------------------------------------------------------------------------------------------------------------------------------+
| # VIDEO: [Watch me talk through the basic example](https://www.loom.com/share/7b96e863438b4d8fb220739b868c48aa?sid=a93a57dd-c82a-4472-97fd-5c50ddc163ee) |
+----------------------------------------------------------------------------------------------------------------------------------------------------------+
...
Learn more about what kinds of things you can search for by looking at the [get_filings()] documentation.

```{r basicexample2, eval=FALSE}
...
See [flag_dupes()] for an explainer on what red flags you might be looking for. [My video](https://www.loom.com/share/7b96e863438b4d8fb220739b868c48aa?sid=a93a57dd-c82a-4472-97fd-5c50ddc163ee) also goes over a few common ones.

```{r basicexample3, eval=FALSE}
...
See [flag_client_registrant_conflict()] for an explainer on what red flags you might be looking for. [My video](https://www.loom.com/share/7b96e863438b4d8fb220739b868c48aa?sid=a93a57dd-c82a-4472-97fd-5c50ddc163ee) also goes over a few common ones.

```{r basicexample4, eval=FALSE}
...
Remember it's on lobbyists to accurately attest what and who they were lobbying on – to there's no exact science. No silver bullet to get every disclosure. The key is to try and find the most common terms. This example already has done some of the

#### Use your reporting to guide you

Take a stab at some terms based on your reporting and then do a quick eyeball of how, what we're looking for, was being defined. Here I started with some terms for the 'revenge tax' or section 899.

```{r complexexample1, eval = FALSE}
...
Take out lobbying disclosures that don't mention these terms in the tax section in case the terms are being used in a different context. Then simplify it into an easier-to-read format (the df we pulled has lots of columns we don't need) before exporting it to a .csv so we can look in excel/google docs

NOTE: I've written it out to a csv but here's where you can start getting creative with LLM, textual analysis, etc. We just need more than an hour for that :)

```{r complexexample2, eval = FALSE}
...
Go back through, and again filter the non-tax disclosures and simplify our dataframe

```{r complexexample4, eval = FALSE}
...
```{r get_filings_example, eval = FALSE}
...
```{r dupes_flag_test_example, eval = FALSE}
...
```{r flagged_conflict_example, eval = FALSE}
...
I welcome help! This package was developed by Chris Cioffi. For questions, issues, or suggestions, please visit: <https://github.com/Lobbying-DisclosuRe/lobbyr>

------------------------------------------------------------------------

## Citation

If you use this package in your research or reporting, please cite the data as being accessed from the U.S. Senate Lobbying Disclosure Act Database and analyzed by the lobbyR package.
